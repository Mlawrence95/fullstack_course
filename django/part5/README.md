...
A complete `views.py` file might then look like:
```python
from django.shortcuts import render
from app1.forms import UserProfileForm, UserProfileInfoForm
...
thus a `base.html` file for the site could easily then use built-in
Django functionality to render views conditioned on whether a user was authenticated
or not. Consider the following `base.html` navbar portion:

```HTML
{% if user.is_authenticated %}
<a href="{% url 'app1:logout' %}" class="nav-item nav-link">Logout</a>
{% else %}
